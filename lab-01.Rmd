...
**Not available, this is becase there are NAs or non-numeric values in the n_2021 colum, so the median cannot be calculated.**
...
**There is a greater difference between visitor numbers for the larger settings and the median than the difference between smaller settings and the median.**
...
proportion_free <- visitors_with_nations %>%
  filter(
      admission == "Free",
      nation == "Scotland",
  )
Scotland_free <- proportion_free %>% count(nation)
mutate (Scotland_free)
glimpse (proportion_free)
